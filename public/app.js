...
function profesionalizarSidebarPOS() { document.querySelectorAll(".sidebar button").forEach(boton => { const etiqueta = limpiarTextoUI(boton.dataset.navLabel || boton.textContent); const modulo = boton.dataset.shellModule || moduloDesdeEtiquetaPOS(etiqueta); boton.dataset.shellModule = modulo; boton.dataset.navLabel = etiqueta; boton.innerHTML = iconoUISVG(iconoModuloPOS(modulo)) + '<span>' + etiqueta + '</span>'; }); }
...
function contactoDesarrolladorPOS() { try { return { nombre: "Desarrollador Nexo POS", telefono: "", whatsapp: "", correo: "", ...JSON.parse(localStorage.getItem(CONTACTO_DESARROLLADOR_KEY) || "{}") }; } catch (error) { return { nombre: "Desarrollador Nexo POS", telefono: "", whatsapp: "", correo: "" }; } }
function abrirContactoDesarrolladorPOS() { cerrarMenuNexoPOS(); const contacto = contactoDesarrolladorPOS(); let modal = document.getElementById("modalContactoDesarrolladorPOS"); if (!modal) { modal = document.createElement("div"); modal.id = "modalContactoDesarrolladorPOS"; modal.className = "modal-personalizado"; document.body.appendChild(modal); } const whatsappLink = contacto.whatsapp ? "https://wa.me/" + String(contacto.whatsapp).replace(/\D/g, "") : ""; modal.innerHTML = '<div class="modal-card contacto-dev-card"><div class="modal-card-header"><div><span>Nexo POS</span><h3>Contacto del desarrollador</h3></div><button type="button" onclick="cerrarContactoDesarrolladorPOS()">Cerrar</button></div><div class="contacto-dev-resumen"><img src="nexo-pos-icon.jpg" alt="Nexo POS"><div><strong>' + (contacto.nombre || "Desarrollador Nexo POS") + '</strong><span>Soporte, cambios y mejoras del sistema</span></div></div><div class="contacto-dev-actions">' + (contacto.telefono ? '<a href="tel:' + contacto.telefono + '">Telefono: ' + contacto.telefono + '</a>' : '<span>Telefono pendiente</span>') + (contacto.correo ? '<a href="mailto:' + contacto.correo + '">Correo: ' + contacto.correo + '</a>' : '<span>Correo pendiente</span>') + (whatsappLink ? '<a href="' + whatsappLink + '" target="_blank" rel="noopener">Abrir WhatsApp</a>' : '<span>WhatsApp pendiente</span>') + '</div><div class="contacto-dev-form"><label>Nombre<input id="contactoDevNombrePOS" value="' + (contacto.nombre || "") + '" placeholder="Ej. Gustavo Diaz"></label><label>Telefono<input id="contactoDevTelefonoPOS" value="' + (contacto.telefono || "") + '" placeholder="Ej. 498 123 4567"></label><label>WhatsApp<input id="contactoDevWhatsappPOS" value="' + (contacto.whatsapp || "") + '" placeholder="Ej. 524981234567"></label><label>Correo<input id="contactoDevCorreoPOS" value="' + (contacto.correo || "") + '" placeholder="correo@ejemplo.com"></label></div><div class="modal-actions-row"><button type="button" onclick="cerrarContactoDesarrolladorPOS()">Cancelar</button><button type="button" class="btn-principal" onclick="guardarContactoDesarrolladorPOS()">Guardar contacto</button></div></div>'; modal.style.display = "flex"; }
